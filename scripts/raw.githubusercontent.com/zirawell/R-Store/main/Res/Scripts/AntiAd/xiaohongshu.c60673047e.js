...
// å®�å†µç…§ç‰‡ä¿�å­˜è¯·æ±‚
  ...
  if (obj?.data?.datas?.length > 0) {
    ...
    // å�Ÿå§‹æ•°æ�®æœ‰é—®é¢˜ å¼ºåˆ¶è¿”å›�æˆ�åŠŸå“�åº”
    obj = {"code": 0, "success": true, "msg": "æˆ�åŠŸ", "data": {"datas": newDatas}};
  }
...
// ä¿¡æ�¯æµ� è§†é¢‘
  ...
  if (url.includes("/v3/")) {
    ...
    if (obj.data?.length > 0) {
        for (let item of obj.data) {
          // æ·»åŠ ä¸‹è½½æŒ‰é’®ï¼ˆå¦‚æ�œæœªå­˜åœ¨ï¼‰
          if (item?.share_info?.function_entries?.length > 0) {
        ...
        // å†™å…¥æœ¬åœ°æŒ�ä¹…åŒ–ç¼“å­˜
        $.setdata(JSON.stringify(videoData), "redBookVideoFeed");
    }
  }
// è§†é¢‘ä¿�å­˜è¯·æ±‚
} else if (url.includes("/note/video/save")) {
  let videoFeed = JSON.parse($.getdata("redBookVideoFeed")); // è¯»å�–æŒ�ä¹…åŒ–å­˜å‚¨
...
function imageEnhance(jsonStr) {
  ...
  if (imageQuality === "original") { // å�Ÿå§‹åˆ†è¾¨ç�‡ï¼ŒPNGæ ¼å¼�çš„å›¾ç‰‡ï¼Œå� ç”¨ç©ºé—´æ¯”è¾ƒå¤§
